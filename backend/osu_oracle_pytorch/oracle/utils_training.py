...
def get_data(
    ...
    """
    Load beatmaps and optionally apply geometric augmentations.
    ...
    chunk_size : int
        Number of database rows fetched at once.

    augment : bool
        Master switch for augmentation.

        False:
            Only original beatmaps are returned.

        True:
            Augmentations specified by augmentation_types are applied.
    ...
    augmentation_types : set[str] | None
        ...
        If None and augment=True:
            All augmentations are enabled.

    augmentation_ids : set[int] | None
        Optional set of beatmap IDs to augment.

        None:
            Augment all beatmaps.

        Set of IDs:
            Only those beatmaps are augmented.
    ...
    beatmap_ids : list
        Database beatmap IDs corresponding to X/y.
    """
    valid_augmentations = {
        "horizontal",
        "vertical",
    ...
    if augmentation_types is None:
        ...
        if invalid:
            raise ValueError(f"Unknown augmentation types: {invalid}. Valid types: {valid_augmentations}")

    # If augmentation is disabled, ignore augmentation settings.
    if not augment:
    ...
    if not augment:
        ...
        print("Augmentation types:", sorted(augmentation_types))

        if augmentation_ids is None:
            print("Augmentation IDs: ALL")
        else:
            print(f"Augmentation IDs: {len(augmentation_ids)} selected")
    ...
    for beatmap_id, vectors, label in zip(beatmap_ids, original_X, original_y):
        # Original
        augmented_X.append(vectors)
        augmented_y.append(label)
        augmented_ids.append(beatmap_id)

        # Determine whether this beatmap should be augmented
        if augmentation_ids is not None and beatmap_id not in augmentation_ids:
            continue
        ...
        if "horizontal" in augmentation_types:
            horizontal = [
                (-x_diff, y_diff, normalized_time_diff, normalized_length)
                for (x_diff, y_diff, normalized_time_diff, normalized_length) in vectors
            ]
    ...
    print(f"Final X length: {len(augmented_X)}")
    print(f"Final y length: {len(augmented_y)}")
    print(f"Final beatmap_ids length: {len(augmented_ids)}")
